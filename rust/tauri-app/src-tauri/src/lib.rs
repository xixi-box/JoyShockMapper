...
fn configure_fly_mouse(
    ...
    core.command(&format!("GYRO_SENS = {sensitivity}"))?;
    core.command(&format!("GYRO_ON = {}", if enabled { hold_button.as_str() } else { "NONE" }))
}
...
pub fn run() {
	...
	let app = tauri::Builder::default()
        ...
        .setup(move |app| {
            app.manage(SettingsState(Mutex::new(read_settings(app.handle()))));
			...
			#[cfg(windows)]
			{
				...
				core.command(&format!("GYRO_SENS = {}", fly.sensitivity)).map_err(std::io::Error::other)?;
				core.command(&format!("GYRO_ON = {}", if fly.enabled { button.as_str() } else { "NONE" })).map_err(std::io::Error::other)?;
				app.manage(core);
			}
